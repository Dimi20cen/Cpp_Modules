...
{
	if (form.getSigned() == 1)		std::cout << name << " cannot sign " << form.getName() << " because it's already signed" << std::endl;
...
void Bureaucrat::executeForm(Form const & form)
{
	if (form.getSigned() == 1)
	{
		if (getGrade() <= form.getGrade_to_exe())
		{
			form.execute(*this);
		}
		else
		{
			std::cout << "lowest grade than needed, will throw exception!" << std::endl;
			throw GradeTooLowException();
		}
	}
	else
		std::cout << form.getName() << " needs to be signed before being executed!" << std::endl;
}